...
from .pygamebutton import button_color, Button, SetAllCursorButtons
from .format_number import NumberFormat
from .wraptext_pygame import wrap_text
...
class Popup:
    ...
    def __init__(self, instance) -> None:
        ...
        self.index               = 0
        self.percent_format      = NumberFormat(self.instance.languages['exponents-number'], decimal_places=1, rounded=False, anchor_decimal_places=True)
    ...
    def __call__(self) -> str | None:
        ...
        while not self.is_animation_up_end:
            ...
            self.instance.screen.fill(self.instance.colors.screen)
            self.instance.showTile()
            self.instance.showBarMenu (justshow=True)
            self.instance.showKeyboard(justshow=True)
            self.instance.show_fps_and_version()
    ...
    def show_how_to_play(self) -> None:
        ...
        box_rect_shadow  = pygame.Rect(box_rect_outline.left + 10 * self.instance.geomatry,                                   box_rect_outline.top + 10 * self.instance.geomatry, box_rect_outline.width,       box_rect_outline.height)
        shadow_surface   = pygame.Surface((box_rect_shadow.width, box_rect_shadow.height))
        wrap_title = wrap_text(self.font_title,            LANG['title'], box_rect.width - 100 * self.instance.geomatry)
        wrap_label = wrap_text(self.font_how_to_play_text, LANG['label'].replace('<TILE-TOP>', '\u200b', 3)
        ...
        for i, ln in enumerate(wrap_title):
        ...
        for i, ln in enumerate(wrap_label):
    ...
    def show_stats(self) -> None:
        ...
        for i, ln in enumerate(wrap_stats_info):
            ...
            surface_stats = self.font_stats_stat_text.render(wrap_stats[i], True, self.instance.colors.popup['text'])
    ...
    def show_hint(self) -> None:
        ...
        box_rect_shadow  = pygame.Rect(box_rect_outline.left + 10 * self.instance.geomatry,                                   box_rect_outline.top + 10 * self.instance.geomatry, box_rect_outline.width,       box_rect_outline.height)
        shadow_surface   = pygame.Surface((box_rect_shadow.width, box_rect_shadow.height))
        wrap_title = wrap_text(self.font_title,   self.kw['title'], box_rect.width - 100 * self.instance.geomatry)
        wrap_label = wrap_text(self.font_message, self.kw['label'], box_rect.width - 10 * self.instance.geomatry)
        ...
        for i, ln in enumerate(wrap_label):
            ...
            self.instance.screen.blit(surface_text, surface_text.get_rect(left=box_rect.left + const.Math.get_center(box_rect.width, surface_text.get_width()), top=self.index + (box_rect.height - surface_text.get_height() * len(wrap_label)) / 2 + i * surface_text.get_height()))
    ...
    def show_daily_coins(self) -> None:
        ...
        shadow_surface       = pygame.Surface((box_rect_shadow.width, box_rect_shadow.height))
        wrap_title = wrap_text(self.font_title,   LANG['title'],   box_rect.width - 100 * self.instance.geomatry)
        wrap_label = wrap_text(self.font_message, LANG['label'].replace('<DAILY-COINS>', str(self.kw['daily_coins']), 1)
