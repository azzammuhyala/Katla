...
class Button(_ButtonInterface):
    ...
    def __init__(
            ...
            image_scale: _RealNumber | _ArgsList | None = None,
        ...
        self.__scaled_image: pygame.Surface = self.image.copy() if isinstance(self.image, pygame.Surface) else None
    ...
    def __render_active_button(self, current_time: int, change_config: bool) -> None:
        ...
        if not self.__button_outside:
            ...
            if isinstance(self.outline_size, _RealNumber):
                pygame.draw.rect(
                    self.surface_screen,
                    self.outline_color.active_color,
                    pygame.Rect(
                        self.rect.left - self.outline_size,
                        self.rect.top - self.outline_size,
                        self.rect.width + self.outline_size * 2,
                        self.rect.height + self.outline_size * 2
                    ),
                    (int(self.outline_size) + 1) if self.outline_size - int(self.outline_size) > 0.1 else int(self.outline_size),
                    **self.borders.draw_rect_kwargs
                )
            ...
            if self.image is not None:
                if self.get_rect_image_kwargs is None:
                    self.surface_screen.blit(self.__scaled_image, self.__scaled_image.get_rect(center=self.rect.center))
                else:
                    self.surface_screen.blit(self.__scaled_image, self.__scaled_image.get_rect(**self.get_rect_image_kwargs))
            ...
            if isinstance(self.outline_size, _RealNumber):
                pygame.draw.rect(
                    ...
                    (self.outline_color.hover_color
                     if (self.outline_color.hover_color is not None) and ismousehover else
                     self.outline_color.inactive_color
                    ),
                    pygame.Rect(
                        self.rect.left - self.outline_size,
                        self.rect.top - self.outline_size,
            ...
            if not self.hide:
                pygame.draw.rect(
                    ...
                    (self.color.hover_color
                     if (self.color.hover_color is not None) and ismousehover else
                     self.color.inactive_color
                    ), self.rect, **self.borders.draw_rect_kwargs
                )
    ...
    def __set_and_validates(self, kwargs: dict | None = None) -> None:
        ...
        _prvt.asserting(isinstance(self.image_scale, _RealNumber | _ArgsList | None), TypeError(f'image_scale: must be RealNumber or ArgsList or (None for not being scaled) not {_prvt.get_type(self.image_scale)}'))
    ...
    def get_private_attr(self, remove_underscore: bool = False) -> dict[str, object]:
        ...
        attr_dict = {
            ...
            '__scaled_image': self.__scaled_image.copy()
        }
    ...
    def scale_image(self) -> None:
        ...
        if isinstance(self.image_scale, _RealNumber | _ArgsList) and self.image is None:
            raise ValueError('image_scale: Cannot transform scale image because image parameters have not been provided')
        elif isinstance(self.image_scale, _RealNumber):
            self.__scaled_image = pygame.transform.scale(self.image, (self.rect.width - self.image_scale, self.rect.height - self.image_scale))
        elif isinstance(self.image_scale, _ArgsList):
            self.__scaled_image = pygame.transform.scale(self.image, self.image_scale)
